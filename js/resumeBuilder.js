...
bio.display = function() {
  ...
  contacts.append(formattedMobile, formattedEmail, formattedGithub, formattedTwitter, formattedLocation);
...
education.display = function() {
  ...
  education.schools.forEach(function(school) {
    var formattedName = HTMLschoolName.replace("%data%", school.name).replace("#", school.url) + HTMLschoolDegree.replace("%data%", school.degree);
    var formattedLocation = HTMLschoolLocation.replace("%data%", school.location);
    var formattedMajor = HTMLschoolMajor.replace("%data%", school.major);
    var formattedDate = HTMLschoolDates.replace("%data%", school.dates);

    entry.append(formattedName, formattedLocation, formattedDate, formattedMajor);
  });

  entry.append(HTMLonlineClasses);
  education.onlineCourses.forEach(function(course) {
    var formattedName = HTMLonlineTitle.replace("%data%", course.title).replace("#", course.url) + HTMLonlineSchool.replace("%data%", course.school);
    ...
    entry.append(formattedName, formattedDate, formattedURL);
  });
...
work.display = function() {
  ...
  work.jobs.forEach(function(job) {
    var formattedJob = HTMLworkEmployer.replace("%data%", job.employer) + HTMLworkTitle.replace("%data%", job.title);
    var formattedLocation = HTMLworkLocation.replace("%data%", job.location);
    var formattedDate = HTMLworkDates.replace("%data%", job.dates);
    var formattedDescription = HTMLworkDescription.replace("%data%", job.description);

    entry.append(formattedJob, formattedLocation, formattedDate, formattedDescription);
  });
};
...
projects.display = function() {
  ...
  projects.forEach(function(project) {
    ...
    entry.append(formattedProject, formattedDescription);
